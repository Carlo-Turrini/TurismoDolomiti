...
public interface CommentoDAO extends JpaRepository<Commento, Long> {
	...
	public List<CommentoCardDto> findCommentiByElemento(@Param("id_elemento")Long idElemento, Pageable pageable); //Altrimenti page
	
	@Query("SELECT COUNT(c) FROM Commento c WHERE c.id = :id_com AND c.elemento.id = :id_el")
	public Integer verificaEsistenzaCommentoRifugio(@Param("id_com")Long idCom, @Param("id_el")Long idEl);
	
	@Query("SELECT COUNT(c) FROM Commento c WHERE c.id = :id_com AND c.utente.id = :id_utente")
	public Integer verificaUtenteCommento(@Param("id_com")Long idCom, @Param("id_utente")Long idUtente);
}
