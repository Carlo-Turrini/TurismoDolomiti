...
public interface FotoDAO extends JpaRepository<Foto, Long> {
	...
	public List<FotoCardDto> findPhotoesByElemento(@Param("id_elemento") Long idElemento);
	...
	public List<FotoSequenceDTO> findPhotoesForSequence(@Param("id_elemento") Long idElemento, Pageable pageable);
	
	@Query("SELECT COUNT(f) FROM Foto f WHERE f.id = :id_foto AND f.elemento.id = :id_el")
	public Integer verificaEsistenzaFotoEl(@Param("id_foto")Long idFoto, @Param("id_el")Long idEl);
	
	@Query("SELECT COUNT(f) FROM Foto f WHERE f.id = :id_foto AND f.utente.id = :id_utente")
	public Integer verificaUtenteFoto(@Param("id_foto")Long idFoto, @Param("id_utente") Long idUtente);
}
